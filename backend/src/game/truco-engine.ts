import type { Card, Suit, Rank, GameState, Trick, GameConfig } from "../types"
...
export class TrucoEngine {
  ...
  static canPlayCard(gameState: GameState, playerId: string, cardId: string): boolean {
    if (gameState.turnPlayerId !== playerId) return false
   ...
   */
  static skipPhase(gameState: GameState): GameState {
    const newState = { ...gameState }

    if (gameState.phase === "flor") {
      newState.phase = "envido"
    } else if (gameState.phase === "envido") {
      newState.phase = "truco"
    } else if (gameState.phase === "truco") {
  ...
  static respondToCall(gameState: GameState, playerId: string, accept: boolean): GameState {
    ...
    if (accept) {
      ...
      if (
        ...
        newState.call = null
        newState.accepted = false
      } else if (gameState.call.type === "truco") {
        // Truco accepted, continue playing
        newState.accepted = true
      }
    } else {
  ...
  static playCard(gameState: GameState, playerId: string, cardId: string): GameState {
    ...
    if (newState.table.plays.length === 2) {
      ...
      if (maxTricks >= 2 || newState.roundNumber === 3) {
        ...
        newState.turnPlayerId = trickWinner
      }
    } else {
  ...
  /**
   * Handle Truco call (updated to work with new call system)
   */
  static callTruco(gameState: GameState, playerId: string): GameState {
    const currentLevel = (gameState.call?.type === "truco" ? gameState.call.level : 0) || 0

    if (currentLevel >= 4) {
      throw new Error("Cannot raise beyond Vale 4")
    }

    const newLevel = (currentLevel + 1) as 1 | 2 | 3 | 4
  ...
  static evaluateTrick(trick: Trick): string {
    ...
    if (comparison > 0) {
      ...
      return play1.playerId // First player wins on tie
    }
  }
  ...
  static calculateHandPoints(gameState: GameState): number {
    if (gameState.call && gameState.accepted && gameState.call.type === "truco") {
      // Truco was called and accepted
      return gameState.call.level + 1 // 1=Truco(2pts), 2=ReTruco(3pts), 3=Vale4(4pts)
    } else if (gameState.call && !gameState.accepted && gameState.call.type === "truco") {
